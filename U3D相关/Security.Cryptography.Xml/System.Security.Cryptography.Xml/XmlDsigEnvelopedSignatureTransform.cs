...
public class XmlDsigEnvelopedSignatureTransform : Transform
{
	...
	private readonly Type[] _outputTypes = new Type[2]
	{
		typeof(XmlNodeList),
		typeof(XmlDocument)
	};
	...
	private void LoadStreamInput(Stream stream)
	{
		...
		{
			throw new CryptographicException(System.SR.Cryptography_Xml_EnvelopedSignatureRequiresContext);
		}
			...
			CanonicalXmlNodeList canonicalXmlNodeList = new CanonicalXmlNodeList();
			{
				foreach (XmlNode inputNode in _inputNodeList)
				{
					...
					try
					{
						...
						if (xmlNode2 == null || num != _signaturePosition)
						{
							canonicalXmlNodeList.Add(inputNode);
						}
					}
