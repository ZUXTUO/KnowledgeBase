...
class ZipRecovery:
    def __init__(self, filename: str):
        ...
        self.data_descriptor = False
        self.skip_size_check = False
        self.structDataDescriptor = b"<4sL2L"
        self.stringDataDescriptor = b"PK\x07\x08"
        self.sizeDataDescriptor = struct.calcsize(self.structDataDescriptor)

        self._DD_SIGNATURE = 0
        self._DD_CRC = 1
        self._DD_COMPRESSED_SIZE = 2
        self._DD_UNCOMPRESSED_SIZE = 3
    ...
    def fdescriptor_reader(self, file, initial_offset):
        ...
        file.seek(offset)
        ddescriptor_raw = file.read(self.sizeDataDescriptor)
        if len(ddescriptor_raw) < self.sizeDataDescriptor:
            print('已找到文件末尾。数据描述符不完整。')
            return None
        
        try:
            ddescriptor = struct.unpack(self.structDataDescriptor, ddescriptor_raw)
            if ddescriptor[self._DD_SIGNATURE] != self.stringDataDescriptor:
                print(f'警告: 数据描述符签名在偏移量 {offset} 处不匹配。预期 {self.stringDataDescriptor}，得到 {ddescriptor[self._DD_SIGNATURE]}')
                return None
            return ddescriptor
    ...
    def repair(self):
        print(f'正在手动尝试读取 {self.filename}。')
        ...
        try:
            ...
            print(f'警告: 根据 zipfile 模块，{self.filename} 不是有效的 zip 文件。将继续手动解析。')
        except Exception as e:
            print(f"读取中心目录时发生意外错误: {e}")

        print(f'正在手动读取 {self.filename} 的 ZIP 条目。')
                ...
                try:
                    ...
                    if data_descriptor_in_use:
                        ...
                        while True:
                            ...
                            temp_data += chunk
                            if self.stringDataDescriptor in temp_data:
                                compressed_data = temp_data.split(self.stringDataDescriptor, 1)[0]
                    ...
                    if data_descriptor_in_use:
                        ddescriptor = self.fdescriptor_reader(f, f.tell())
                        if ddescriptor:
                            zi.compress_size = ddescriptor[self._DD_COMPRESSED_SIZE]
                            zi.file_size = ddescriptor[self._DD_UNCOMPRESSED_SIZE]
                            zi.CRC = ddescriptor[self._DD_CRC]
                            print(f'已从数据描述符更新大小: 压缩={zi.compress_size}, 未压缩={zi.file_size}, CRC={zi.CRC:08x}')
                        else:
                            print(f"警告: 无法读取 {fname} 的数据描述符。如果可用，将使用头部值。")

                    if not self.skip_size_check and len(data) != zi.file_size:
                        print(f"错误: {fname.decode('utf-8') if isinstance(fname, bytes) else fname} 的解压缩数据大小不匹配! 预期 {zi.file_size}，得到 {len(data)}。这表示已损坏。")
                        self.write_data(fname, data) 
                        continue
                    ...
                    if data_descriptor_in_use:
                        f.seek(f.tell() + self.sizeDataDescriptor)
                    else:
    ...
    try:
        with open(file_path, 'rb') as f:
            ...
            if header.startswith(b'\x37\x7A\xBC\xAF\x27\x1C'):
                return '7z'
            elif header.startswith(b'\x50\x4B\x03\x04') or header.startswith(b'\x50\x4B\x05\x06') or header.startswith(b'\x50\x4B\x07\x08'):
                return 'zip'
    except Exception as e:
        print(f"无法检测文件类型: {e}")
    return None
...
def main():
    if len(sys.argv) < 2:
        print("用法: python script.py <文件路径> [输出目录(仅7z)]")
        return
    ...
    if file_type == '7z':
        print(f"检测到 7z 文件: {file_path_str}")
        recovery = SevenZipRecovery(file_path_str, output_dir)
        success = recovery.repair()
    elif file_type == 'zip':
        print(f"检测到 ZIP 文件: {file_path_str}")
        recovery = ZipRecovery(file_path_str)
        success = recovery.repair()
    else:
        print(f"无法识别文件类型或不支持此文件类型: {file_path_str}")
        print("目前仅支持 .7z 和 .zip 文件。")
        return
